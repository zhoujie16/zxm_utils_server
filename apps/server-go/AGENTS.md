...
本文件适用于 `apps/server-go`。除本文件外，仍需遵循仓库根目录 `AGENTS.md`。
...
- 在仓库根目录运行 `yarn dev:server-go` 或 `yarn dev:server` 启动服务。
- 在 `apps/server-go` 内运行 `go run ./cmd/server/main.go` 启动服务。
- 在 `apps/server-go` 内运行 `go test ./...` 执行后端测试。
- 在仓库根目录运行 `yarn server-go:build` 或 `yarn server:build` 交叉编译服务。
